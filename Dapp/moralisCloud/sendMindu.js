Moralis.Cloud.define("sendMindu", async (request) => {
	...
	const cAddress = config.get("MINDU_ADDRESS", {
		useMasterKey: true,
	});

	const ABI = config.get("MINDU_ABI", {
		useMasterKey: true,
	}).abi;
	const contract = new web3.eth.Contract(ABI, cAddress);
	let valor = web3.utils.toWei(request.params.amount, "ether");
	const tx = {
		...
		data: contract.methods
			.transfer(request.params.address, valor)
			.encodeABI(),
	};
